...
microacc = sum(tp+tn)/sum(tp+tn+fp+fn);
...
values = [tp; fp; fn; tn; prec; sens; spec; acc; f1]; values(isnan(values)) = 0;

% Macro-average
macroAVG = mean(values, 2); macroAVG(8) = sum(tp)/sum(confusion,"all");
