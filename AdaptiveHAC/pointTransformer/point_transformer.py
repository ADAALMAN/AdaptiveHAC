...
<<<<<<< f9310f85295fdd2bd3bb31c64ee56c6b3f1606fd
...
>>>>>>> 141301d07389e2610aa2cb4e7d4943ceb5d20714
import logging
...
=======
def test_PC(args, model, fusion, TEST_PC):
...
def test(args, model, fusion, TEST_PC, PC_PATH):
    ...
    y_pred_all = []
    y_true_all = []
    activities = ["N/A", "Walking", "Stationary", "Sitting down","Standing up (sitting)",
                    "Bending (sitting)","Bending (standing)",
                    "Falling (walking)","Standing up (ground)","Falling (standing)"]
    # classify segments
    for dataset in tqdm(TEST_PC, total=len(TEST_PC)):
            
        TEST_DATASET = PCFileModelNetDataLoader(PC_names=dataset, root=f"{PC_PATH}", npoint=args.num_point)
        testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=args.batch_size, shuffle=False, num_workers=4)
    
        if isinstance(dataset, str):
            y_true = int(dataset[7])
        elif isinstance(dataset[0], str):
            y_true = int(dataset[0][7])
