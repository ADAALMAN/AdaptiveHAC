...
for i=1:length(namesShort)
    ...
    for file_path = strcat(file_names,nameShort)
        ...
        PBC_avg = zeros([size(PBC,2),size(PBC,2),size(PBC,1)]);
        PBC_score = zeros([size(PBC,2),size(PBC,2)]);
        ...
        for i=1:size(PBC,2)
            for j=1:size(PBC,2)
                if i<j
                    PBC_avg(i,j,:) = mean([PBC(:,i),PBC(:,j)],2);
                elseif i == j
                    PBC_avg(i,j,:) = PBC(:,i);
                end
                % H-time stamps
                d1 = reshape(PBC_avg(i,j,:),[],1);
                [~, s1, ~] = lagSearch(d1);
                tr1 = sig2timestamp(s1,t);
                % compute score
                if i <= j; [PBC_score(i,j), ~] = perfFuncLin(tr1,tr2); end
            end
        end
        ...
        tr1 = sig2timestamp(s1,t);
        % compute score
        [PBC_avg_score,~] = perfFuncLin(tr1,tr2);
